  ...
  reducers: {
    ...
    removeExpiredSystemMessages: {
      reducer: (state, action) => {
        ...
        // Only replace the array when something actually expired, otherwise
        // the periodic sweep would churn the reference and re-render every
        // subscriber on each tick.
        if (next.length !== state.items.length) {
          state.items = next;
        }
      },
      // Resolve the clock in the action creator so the reducer stays pure.
